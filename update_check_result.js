...
let content = fs.readFileSync('check-result.html', 'utf8');

const css_old = /        \.result-section \{[\s\S]*?\.fail \{ color: #ef4444; font-weight: 600; \}/m;
const css_new = `        .result-section {
            display: none;
            margin-top: 30px;
            background: #fff;
            padding: 30px;
...
const html_old = /        <div class="result-section" id="resultSection">[\s\S]*?<\/div>\s*<\/div>/;
const html_new = `        <div class="result-section" id="resultSection">
            <!-- Marksheet Header -->
...
const js_old = /                \/\/ 2\. Display student info[\s\S]*?}\)\.join\(''\);\n                }/;
const js_new = `                // 2. Display student info
...
fs.writeFileSync('check-result.html', content, 'utf8');
console.log('Successfully updated check-result.html!');
